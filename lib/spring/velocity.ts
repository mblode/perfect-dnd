...
/** Frozen: this is handed out by reference on every zero-velocity sample, so a
 * caller mutating it would corrupt every later reading. */
const AT_REST: Point = Object.freeze({ x: 0, y: 0 });

export interface VelocityTracker {
  /**
   * Record a pointer position and return the velocity in px/second across the
   * window. Returns zero until two samples span real time.
   */
...
export const createVelocityTracker = (): VelocityTracker => {
  ...
  return {
    sample(point, now, windowMs) {
      ...
      // A duplicated or hand-edited timestamp can still produce a non-finite
      // result, and NaN here would propagate into the spring forever.
