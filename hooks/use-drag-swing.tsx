...
import { findOverlayCard, LIFTED_SHADOW, SHADOW_FADE_MS } from "@/lib/dnd/dom";
 ...
 * measured velocity, and with it the tilt, decay to zero.
 */
 ...
 * there is no `onDragStart` handler here.
 *
...
export function useDragSwing({
  ...
  /** Record a pointer position and steer the tilt spring at the resulting velocity. */
  ...
  const handleDragMove = useCallback(
    (event: DragMoveEvent) => {
      ...
      // from the activation point, so the fallbacks reconstruct the pointer
      // rather than reusing a stale absolute position.
   ...
   * without a visible jump. `getBoundingClientRect` reports the *transformed*
   * box, which the tilt inflates, so the untransformed layout size is
   * re-centred on the transformed centre.
   */
  ...
  const handleRelease = useCallback(
    (settle: boolean) => {
      ...
      // Read the card before reporting: the caller's state change unmounts
      // this overlay, and the element goes with it.
